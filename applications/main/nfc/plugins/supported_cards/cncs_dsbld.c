...
} __attribute__((packed)) DsbldData;

static const MfClassicKeyPair dsbld_keyset_v1[] = {
    ...
    {.a = 0xffffffffffff, .b = 0xffffffffffff}, {.a = 0xffffffffffff, .b = 0xffffffffffff},
    {.a = 0xffffffffffff, .b = 0xffffffffffff}, {.a = 0xffffffffffff, .b = 0xffffffffffff},
    {.a = 0xffffffffffff, .b = 0xffffffffffff}, {.a = 0x3a8a139c20b4, .b = 0x8818a9c5d406},
    ...
    {.a = 0x0eb23cc8110b, .b = 0x04dc35277635}, {.a = 0xffffffffffff, .b = 0xffffffffffff},
    {.a = 0xffffffffffff, .b = 0xffffffffffff}, {.a = 0xffffffffffff, .b = 0xffffffffffff},
    {.a = 0xffffffffffff, .b = 0xffffffffffff}, {.a = 0xffffffffffff, .b = 0xffffffffffff},
    {.a = 0xffffffffffff, .b = 0xffffffffffff}, {.a = 0xffffffffffff, .b = 0xffffffffffff},
    {.a = 0xffffffffffff, .b = 0xffffffffffff}, {.a = 0xffffffffffff, .b = 0xffffffffffff},
...
static const MfClassicKeyPair dsbld_keyset_v2[] = {
...
static bool dsbld_verify(Nfc* nfc) {
    ...
    // Both keysets share the same sector 15 key A
    bit_lib_num_to_bytes_be(dsbld_keyset_v1[15].a, sizeof(MfClassicKey), mf_key.data);
    ...
    // Disabled: 0x24 0x23, Accompany disabled: 0x24 0x24
    if(block_data.data[0] != 0x24) return false;
    if(block_data.data[1] != 0x23 && block_data.data[1] != 0x24) return false;

    return true;
}

static bool dsbld_read(Nfc* nfc, NfcDevice* device) {
    furi_assert(nfc);
    furi_assert(device);
    ...
    do {
        ...
        const MfClassicKeyPair* keysets[] = {dsbld_keyset_v1, dsbld_keyset_v2};
        uint8_t num_keysets = 2;

        const MfClassicKeyPair* keys = NULL;

        for(uint8_t k = 0; k < num_keysets; k++) {
...
static bool dsbld_parse(const NfcDevice* device, FuriString* parsed_data) {
    furi_assert(device);
    const MfClassicData* data = nfc_device_get_data(device, NfcProtocolMfClassic);
    bool parsed = false;
    DsbldData ticket_data = {0};
    ...
    if(key_a != dsbld_keyset_v1[8].a) return false;

    // Reject BEPK: sector 15 key A is 0xffffffffffff on BEPK
    ...
    // Reject student (0x05) and school (0x06)
    if(data->block[61].data[1] == 0x05) return false;
    if(data->block[61].data[1] == 0x06) return false;

    // Must be of type: 0x24 0x23 or 0x24 0x24
    if(data->block[61].data[0] != 0x24) return false;
    if(data->block[61].data[1] != 0x23 && data->block[61].data[1] != 0x24) return false;
    ...
    FuriString* card_str = furi_string_alloc_set_str("9643 2178 ");
    ...
    const char* card_type = (data->block[61].data[1] == 0x23) ? "Disabled" : "Accompany Disabled";
...
static const NfcSupportedCardsPlugin dsbld_plugin = {
...
__attribute__((used)) const FlipperAppPluginDescriptor* cncs_dsbld_plugin_ep() {
    static const FlipperAppPluginDescriptor plugin_descriptor = {
        .appid = NFC_SUPPORTED_CARD_PLUGIN_APP_ID,
        .ep_api_version = NFC_SUPPORTED_CARD_PLUGIN_API_VERSION,
        .entry_point = &dsbld_plugin,
    };
